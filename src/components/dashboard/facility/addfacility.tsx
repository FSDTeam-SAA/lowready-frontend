...
export default function AddFacilityPage() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      // Add care services
      formData.careServices?.forEach(service => {
        formDataToSend.append('offers', service);
      });

      // Add amenity services
      formData.amenityServices?.forEach(service => {
        formDataToSend.append('services', service);
      });

      // Add about information
      formDataToSend.append('about', formData.about?.description || '');
      formDataToSend.append('videoTitle', formData.about?.videoTitle || '');
      formDataToSend.append('videoDescription', formData.about?.videoDescription || '');

      // Add available times
      formData.availableTimes?.forEach(dayTime => {
        dayTime.times.forEach(time => {
          formDataToSend.append('availableTime', time);
        });
      });
      ...
      const response = await fetch('http://localhost:5000/api/v1/facility/create', {
        method: 'POST',
        body: formDataToSend,
      });
                ...
                <div className="lg:col-span-1">
                  <div className="bg-white rounded-lg p-6 border border-gray-200 sticky top-8">
                    ...
                    <div className="grid grid-cols-3 gap-2">
                      ...
                      {Array.from({ length: Math.max(0, 3 - selectedImages.length) }).map((_, index) => (
                        <div
                          key={`empty-${index}`}
